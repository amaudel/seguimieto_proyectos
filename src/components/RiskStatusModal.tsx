...
export const RiskStatusModal: React.FC<RiskStatusModalProps> = ({
  isOpen,
  onClose,
  onConfirm,
  status,
  ...
  const isMitigado = status === 'Mitigado';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isCerrado && !trimmedNote) {
      setError('Debes ingresar obligatoriamente una nota explicando el motivo del cierre del riesgo o impedimento.');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Error al actualizar el estado del riesgo.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="relative bg-white rounded-2xl border border-slate-200 shadow-xl max-w-md w-full overflow-hidden z-10">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
          <div className="flex items-center gap-2">
            {isMitigado ? (
              <CheckCircle className="w-5 h-5 text-emerald-650" style={{ color: '#059669' }} />
            ) : (
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="text-xs text-slate-600 leading-relaxed font-medium">
            {isMitigado && (
              <p>
                Vas a marcar este riesgo/impedimento como <strong className="text-emerald-700 bg-emerald-50 px-1 py-0.5 rounded">Mitigado</strong>.
                Puedes agregar una nota u observación sobre la estrategia de mitigación ejecutada (opcional).
              </p>
            )}
            {isCerrado && (
              <p>
                Vas a marcar este riesgo/impedimento como <strong className="text-sky-700 bg-sky-50 px-1 py-0.5 rounded">Cerrado</strong>.
                Esta acción requiere que expliques de forma obligatoria los motivos o la resolución final.
              </p>
            )}
          </div>
          ...
          <div>
            <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
              Observación / Comentario {isCerrado ? <span className="text-red-500">*</span> : <span className="text-slate-400">(Opcional)</span>}
            </label>
            <div className="relative">
              <FileText className="absolute left-3 top-3 w-4 h-4 text-slate-400" />
              <textarea
                rows={3}
                placeholder={isCerrado ? "Explica detalladamente cómo se resolvió la amenaza..." : "Añade notas del tratamiento ejecutado, etc."}
                value={newNote}
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer ${
              isMitigado ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-sky-600 hover:bg-sky-700'
